...
class Generator(object):
	...
	def __init__(file_path):
		...
		_generate_options = _flow_graph.get_option('generate_options')
		if self._generate_options == 'hb':
		   self._mode = HIER_BLOCK_FILE_MODE
		   dirname = HIER_BLOCKS_LIB_DIR
		else:
	        self._mode = TOP_BLOCK_FILE_MODE
                _mode = TOP_BLOCK_FILE_MODE
		#dirname = os.path.dirname(file_path)
		#handle the case where the directory is read-only
		#in this case, use the system's temp directory
		#if not os.access(file_path, os.R_OK):
		        #dirname = tempfile.gettempdir()
		#filename = _flow_graph.get_option('id') + '.py'
		        #_file_path = os.path.join(dirname, filename)
		#_file_path = os.path.join(file_path) 

	def get_file_path(file_path): return file_path
